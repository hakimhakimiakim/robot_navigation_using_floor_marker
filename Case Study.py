...
# CONFIG
# ============================
BASE_PATH = r"C:\Users\Afiq\OneDrive\Desktop\Case Study"
...
# ============================
# MARKER DETECTION + PREPROCESSING
# ============================
for root, _, files in os.walk(MARKER_FOLDER):
    for file in files:
        ...
        cv2.imshow("Marker Preprocessing",
                    np.hstack([g, b, e, bi]))
        cv2.waitKey(0)
        cv2.destroyWindow("Marker Preprocessing")
        ...
        if ids is not None:
            ...
            for marker_id in ids:
                ...
                cv2.putText(img,
                            f"ID {marker_id[0]} ({area})",
                            (w-350,40),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.9,(0,255,0),2)
                print(f"✅ Marker {marker_id[0]} -> {area}")
        else:
            cv2.putText(img,"NO MARKER DETECTED",
                        (30,50),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        1.2,(0,0,255),3)
